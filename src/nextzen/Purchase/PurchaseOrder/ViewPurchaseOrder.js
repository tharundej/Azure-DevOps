...
import { getPoGenaratorAPI } from 'src/api/Accounts/PurchaseOrder';
import Iconify from 'src/components/iconify';

export default function ViewPurchaseOrder({ currentData, handleClose, getTableData,getPoGenarator }) {
  ...
  const handleChange = (event, newValue) => {
    ...
    if (newValue === '4') {
      if (currentData?.poNumber) {
        getPoGenarator(currentData?.poNumber);
      }
    }
  };
            ...
            <TabContext value={value}>
              <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
                <TabList onChange={handleChange} aria-label="lab API tabs example">
                  ...
                  <Tab label="Download Purchase Order" value="4" />
                </TabList>
              </Box>
              <TabPanel value="1">
                <Table sx={{ minWidth: 650 }} aria-label="simple table">
                  <TableBody>
                    <TableRow>
                      ...
                      <TableCell style={StyledTableCell} onClick={() => getPoGenarator(currentData?.poNumber)}>
                        {currentData?.poNumber} <Iconify style={{ color: 'red', marginLeft: '25px', }} icon="mingcute:pdf-fill" />
                      </TableCell>
                    </TableRow>
                    <TableRow>
